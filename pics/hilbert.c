...
void setmap(int width, int map[]) {
int x, y, inc, over;
for (x = 9; x<79 ; x++) {
	for (y = 9; y<79; y++) {
	map[(y-9)*70+x-9] = pix[x][y] + pix[x-1][y] + (pix[x][y]) + (pix[x][y]) + (pix[x][y]) + (pix[x][y]) + (pix[x][y]) + (pix[x][y]) + (pix[x][y]);
	
	/* map[(y-9)*70+x-9]=pix[x][y];
		for (inc = 1; inc<(2*width); inc++) {
		over++;
		map[(y-9)*70+x-9]+=pix[x-inc][y]<<1;
		};
		
		for (inc = 1; inc<(2*width)+1; inc++) {
		over++;
		map[(y-9)*70+x-9]+=pix[x][y-inc]<<over;
		};
		for (inc = 1; inc<(2*width)+1; inc++) {
		over++;
		map[(y-9)*70+x-9]+=pix[x+inc][y]<<over;
		};
		for (inc = 1; inc<(2*width)+1; inc++) {
		over++;
		map[(y-9)*70+x-9]+=pix[x][y+inc]<<over;
...
bool isUnique(int map[]) {
int x, y, ind, uniqueness;
for (x = 1; x<71 ; x++) {
	for (y = 1; y<71; y++) {
		for (ind = 0; ind<4900; ind++) {
		uniqueness+=map[(y-1)*70+x-1]==map[ind];
		};
	};
};
printf("%d\n", uniqueness);
